...
public class CartItemRepository : ICartItemRepository
{
    ...
    public CartItemRepository(UpstashRedisClient redis)
    {
        _redis = redis;
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await _redis.SetAsync(GetCartItemKey(cartItem.Id), json);
        await _redis.SAddAsync(GetUserCartKey(cartItem.UserId), cartItem.Id);
        await _redis.SetAsync(GetUserCourseKey(cartItem.UserId, cartItem.CourseId), cartItem.Id);
    ...
    public async Task<CartItem?> GetByIdAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var json = await _redis.GetAsync(GetCartItemKey(id));
    ...
    /// <summary>
    /// Check if course exists in user cart
    /// </summary>
    public async Task<bool> ExistsInCartAsync(
        string userId,
        string courseId,
        CancellationToken cancellationToken = default)
    {
        var cartItemId = await _redis.GetAsync(GetUserCourseKey(userId, courseId));
    ...
    public async Task<IEnumerable<CartItem>> GetUserCartItemsAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var cartItemIds = await _redis.SMembersAsync(GetUserCartKey(userId));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await _redis.SetAsync(GetCartItemKey(id), json);
        await _redis.SRemAsync(GetUserCartKey(cartItem.UserId), id);
        await _redis.DeleteAsync(GetUserCourseKey(cartItem.UserId, cartItem.CourseId));
    ...
    public async Task<int> GetItemCountAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var count = await _redis.SCardAsync(GetUserCartKey(userId));
        return (int)count;
    }
}